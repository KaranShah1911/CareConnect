...
const Doctors = () => {
  ...
  useEffect(() => {
    try {
      axios
        .get(`${API_URL}/doctor/list`)
        .then((response) => {
          const responsedoctors = response.data.doctors;
          console.log(responsedoctors);
          setCategories([
            "All",
            ...new Set(responsedoctors.map((doctor) => doctor.specialization.toUpperCase())),
          ]);
  ...
  const applyFilter = (category) => {
    ...
    if (category === "All") {
      ...
      setFilterDoctors(
        doctors.filter((doctor) => doctor.specialization.toLowerCase() === category.toLowerCase())
      );
    }
    handlePageChange(1);
  };
    ...
    <section className="bg-white min-h-screen flex flex-col items-center pb-5 pt-24">
            ...
            >
              {categories.map((category, index) => (
                <button
                  ...
                  className={`block h-[40px] w-[230px] border pl-4 pr-2 mb-3 rounded-md text-start text-sm transition-all ${
                    specialty === category
              ...
              <button
                ...
                onClick={() => {
                  ...
                  navigate(`/appointment?doctorId=${doctor._id}`, {
                    state: { doctor, similarDoctors : filterDoctors.filter(doc => doc.specialization.toLowerCase() === doctor.specialization.toLowerCase()) },
                  });
                }}
      ...
      <div className="flex items-center gap-2 mt-5">
        <button
          ...
          className="flex items-center gap-2 border-1 p-2 rounded-xl cursor-pointer bg-[#5C67F2] text-white disabled:opacity-50"
        >
          <FaArrowLeft />
          <span>Previous</span>
        </button>
        ...
        <button
          ...
          className="flex items-center border-1 p-2 rounded-xl cursor-pointer bg-[#5C67F2] text-white disabled:opacity-50"
        >
          <span>Next</span>
          <FaArrowRight />
        </button>
